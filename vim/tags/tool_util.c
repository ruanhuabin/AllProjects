...
double* compute_grid_bound(int nb, const double *bnds, const int *npts, int *grid_size, const char *center)
{
  ...
  if(NULL == grid1)
  {
  	printf("Grid1: Memory allocate failed, expect size: %d bytes\n", nb*sizeof(double));
  }
  else
  {
  	printf("Grid1: Memory allocate success, allocate size: %d bytes\n", nb*sizeof(double));
  }
  grid1[0] = 1;
  n = 0;
  for(i=1; i<nb; i++) {
  ...
  *grid_size = n*refine;
  ...
  if(NULL == tmp)
  {
  	printf("tmp: Memory allocate failed, expect size: %d bytes\n", np*sizeof(double));
  }
  else
  {
  	printf("tmp: Memory allocate success, allocate size: %d bytes\n", np*sizeof(double));
  }
  grid  = (double *)malloc((*grid_size+1)*sizeof(double));
  if(NULL == grid)
  {
  	printf("Grid: Memory allocate failed, expect size: %d bytes\n", (*grid_size + 1)*sizeof(double));
  }
  else
  {
  	printf("Grid: Memory allocate success, allocate size: %d bytes\n", (*grid_size + 1)*sizeof(double));
  }
  grid2 = (double *)malloc(np*sizeof(double));
  if(NULL == grid2)
  {
  	printf("Grid2: Memory allocate failed, expect size: %d bytes\n", np*sizeof(double));
  }
  else
  {
  	printf("Grid2: Memory allocate success, allocate size: %d bytes\n", np*sizeof(double));
  }
